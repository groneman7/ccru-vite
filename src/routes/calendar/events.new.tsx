...
function RouteComponent() {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            const timeEnd = value.timeEnd ? dayjs(`${value.date} ${value.timeEnd}`).toISOString() : undefined;
            ...
            // TODO: HARDCARDED ID
            await createEvent({ createdBy: "j5799tyr8jpzygb3hb2rae2zs17tbwdz" as Id<"users">, ...test });
        },
    });
                                ...
                                return (
                                    <Field data-invalid={isInvalid}>
                                        ...
                                        <DatePicker
                                            ...
                                            onChange={(value) => field.handleChange(dayjs(value).format("YYYY-MM-DD"))}
                                        />
                                        {isInvalid && <FieldError errors={field.state.meta.errors} />}
                                    </Field>
                                );
                                                            ...
                                                            return (
                                                                ...
                                                                <InputGroupCombobox
                                                                    options={getAllPositions?.filter(
                                                                        (item) =>
                                                                            item._id ===
                                                                                field.state.value[i].positionId ||
                                                                            !field.state.value
